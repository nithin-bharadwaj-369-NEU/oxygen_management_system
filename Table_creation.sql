...
create or replace procedure table_creation(table_query IN VARCHAR2)
IS
BEGIN
...
DECLARE
         ...
         row_cnt NUMBER;
     BEGIN
        dbms_output.put_line('Firing RENTAL PRICE Procedure');
        t_name:='RENTAL_PRICE';
        select count(*) into row_cnt from user_tables where table_name = t_name;
        --dbms_output.put_line(row_cnt);
        IF(row_cnt > 0) THEN
                dbms_output.put_line('Table RENTAL PRICE exists');
        ELSE
            BEGIN
...
DECLARE
     ...
     BEGIN
        ...
        --dbms_output.put_line(row_cnt);
        IF(row_cnt > 0) THEN
                dbms_output.put_line('Table PERMISSION exists');
        ELSE
            BEGIN
