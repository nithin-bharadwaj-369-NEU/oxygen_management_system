...
BEGIN
         ...
         select count(*) into user_exist from dba_users where username=user_name_to_create;
            dbms_output.put_line('>>> User exist status ' || user_exist);
        ...
        EXECUTE IMMEDIATE 'grant execute on sys.dbms_crypto ' || user_name_to_create;
        dbms_output.put_line('>>> Granting access to crypto package');
        EXECUTE IMMEDIATE 'grant execute on sys.dbms_crypto ' || user_name_to_create;
        dbms_output.put_line('>>> Granting access to Random package');
        EXECUTE IMMEDIATE 'grant execute on sys.dbms_random ' || user_name_to_create;
